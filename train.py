...
dataset_train = Places3(args.root, args.mask_root, img_tf, mask_tf, 'train')
...
for i in tqdm(range(start_iter, args.max_iter)):
    ...
    image, mask, gt,image2 = [x.to(device) for x in next(iterator_train)]


    output, _ = model(image2, mask)
    loss_dict = criterion(image2, mask, output, gt)
